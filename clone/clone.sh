...
echo "  task env:migrate            # .env.docker.local -> .env.symfony.migrated"
echo "  # review, then: mv .env.symfony.migrated .env.symfony"
echo "  task env:init               # fill in any missing per-service env files"
echo "  task up                     # bring the v3 stack up"
echo "  task console -- --user deploy app:update   # migrate the DB schema to v3"
echo "See UPGRADE.md for the full 1.x -> 3.x recipe."
