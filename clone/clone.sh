...
echo
echo "Then convert this SAME clone in place to v3 (clone-shaped UPGRADE.md; read it for the why):"
echo
echo "  0. Export env: task -t clone/Taskfile.yml v1:env-migrate   # WHILE v1 is up, on 2.8.0+ images"
echo "               #   Set COMPOSE_VERSION_API=2.8.0 in .env.docker.local + re-run v1:up if needed."
echo "               #   Runs the 2.8 app:utils:convert-env-to-3x in the v1 api container -> writes"
echo "               #   .env.symfony.migrated (env + admin/client config.json). env:migrate has NO"
echo "               #   sed fallback now — this export is REQUIRED."
echo "               task -t clone/Taskfile.yml v1:down            # then stop the v1 stack"
echo "  1. Env:      task env:migrate   # splits the infra advisory out of .env.symfony.migrated"
echo "               mv .env.symfony.migrated .env.symfony"
echo "               task env:init       # create the per-service files (leaves .env.symfony alone)"
echo "               #   Edit .env (OS2DISPLAY_VERSION_API, COMPOSE_PROFILES, keep COMPOSE_PROJECT_NAME)"
echo "               #   and distribute .env.symfony.infra-advisory (COMPOSE_*->.env, PHP_*->.env.php,"
echo "               #   NGINX_*->.env.nginx, MARIADB_*->.env.mariadb). External create-db DB, so the"
echo "               #   bundled-MariaDB 10->11 step does not apply — leave DATABASE_URL serverVersion."
