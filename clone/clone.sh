...
echo "               #   Drop the COMPOSE_*/INTERNAL_* block it leaves (-> .env). The sed path covers"
echo "               #   only the app env; convert the admin/client config.json separately with the"
echo "               #   2.8 cmd app:utils:convert-config-json-to-env (--type=admin|client <file|url>),"
echo "               #   which prints ADMIN_*/CLIENT_* lines for .env.symfony. For the full converter"
echo "               #   flow, run the v1 stack on 2.8 images per UPGRADE.md's pre-upgrade checklist."
